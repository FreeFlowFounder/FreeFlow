...
contract Campaign {
    ...
    address public factory;          // Factory contract - platform owner can collect fees
    uint256 public goal;
    ...
    modifier onlyPlatformOwner() {
        require(msg.sender == getPlatformOwner(), "Not platform owner");
        _;
    }
    ...
    // Platform owner can collect fees
    function collectETHFees(address to) public onlyPlatformOwner {
    ...
    // Platform owner can collect fees
    function collectTokenFees(address token, address to) public onlyPlatformOwner {
    ...
    // Platform owner can collect all fees
    function collectAllFees(address to) external onlyPlatformOwner {
        collectETHFees(to);
    ...
    // Get current platform owner from factory
    function getPlatformOwner() public view returns (address) {
        return ICampaignFactory(factory).owner();
    }
